...
/*
 * Hello, I am sheepy0125 and I have modified this code, which is under the BSD 3-Clause license!
 * I do not endorse nor promote any products derived from this code in any capacity whatsoever.
 *
 * This is basically the Wireless UART example.
 * I've touched the `BleApp_SendUartStream` function along with the custom GATT service in
 * `gatt_db.h` and `gatt_uuid128.h`.
 *
 * Soon, I will meander my way down onto the accelerometer readings.
 * For now, though, I've yet to figure them out. (logan plz help)
 */
...
#define mAppUartBufferSize_c /*244*/  			gAttMaxWriteDataSize_d(gAttMaxMtu_c) /* Local Buffer Size */
...
static uint16_t mAppUartBufferSize = mAppUartBufferSize_c; /* 244 */
/************************************************************************************
 *************************************************************************************
...
static bool_t BleApp_CheckScanEvent (gapScannedDevice_t* pData)
{
    ...
    while (index < pData->dataLength)
    {
        ...
        /* Move on to the next AD element type */ // fixed typo :)
...
static void BleApp_SendUartStream(deviceId_t peerDeviceId, uint8_t *pRecvStream, uint8_t streamSize)
{
    uint16_t  handle;
    uint16_t  handleService;
    bool_t isNotifActive;

    /* Get handle of characteristic */
    if (GattDb_FindServiceHandle(0x0001, gBleUuidType128_c, uuid_service_wireless_uart, &handleService) != gBleSuccess_c)
    	return;
    if (GattDb_FindCharValueHandleInService(handleService, gBleUuidType128_c, uuid_uart_notify_stream, &handle) != gBleSuccess_c)
    	return;

    /* Update characteristic value with the value */
    if (GattDb_WriteAttribute(handle, streamSize, pRecvStream) != gBleSuccess_c)
    	return;
